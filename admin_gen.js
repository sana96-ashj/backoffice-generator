...
Object.keys(schema.definitions).forEach((table) => {
    ...
    function ${table[0].toUpperCase() + table.substring(1)}() {
        ...
        function search(value,field){
            ...
            searchTimeOut = setTimeout(() => {
                axios.get('/api/${table}/search?text='+value).then((res) => {
    ...
    async function upsert(req,res) {
        let id = req.body.id || '';
        delete req.body.id;
        console.log('look hereeeeee',req.body)
...
function itterateFields(table, exec) {
    //  console.log(schema.definitions[table].properties)
    ...
    return itterateFields(table, (prev, current) => {
        ...
        if (['createdAt', 'updatedAt', 'lastLogin'].includes(current)) {
            ...
            return `${prev || ''}
                ...
                <Form.Control name="${current}" type="${schema.definitions[table].properties[current].type}" placeholder="Enter ${current}" />
                </Form.Group>
                \n`
        }
           ...
           ${currrentField.enum.reduce((total, e) => {
            ...
            return `${prev || ''}
            <input className="form-control" placeHolder="${current}"  name="search${current}" onChange={(e)=>{search(e.target.value,'${current}')}}/>
